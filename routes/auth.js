...
passport.use(new DiscordStrategy({
  clientID: config.discord?.clientId || process.env.DISCORD_CLIENT_ID,
  ...
  try {
    ...
    let user = users.find(user => user.discordId === profile.id);
    
    if (!user) {
      // If no user exists with this Discord ID, check if a user exists with the same email
      if (profile.email) {
        user = users.find(user => user.email === profile.email);
        ...
        if (user) {
          ...
          console.log(`Linked Discord account to existing user: ${user.username}`);
        } else {
          // Create a new user account with Discord credentials
          const userId = uuidv4();
          const username = profile.username + '#' + Math.floor(1000 + Math.random() * 9000); // Unique username
          ...
          const newUser = { 
            ...
            verified: true, // Discord accounts are pre-verified
          ...
          await db.set('users', users);
          user = newUser;
          console.log(`Created new user from Discord: ${username}`);
        }
      } else {
        // If Discord didn't provide an email, create a new account anyways
        const userId = uuidv4();
        const username = profile.username + '#' + Math.floor(1000 + Math.random() * 9000);
        ...
        await db.set('users', users);
        user = newUser;
        console.log(`Created new user from Discord (no email): ${username}`);
      }
    } else {
      // Update Discord information if user already exists
      user.discordUsername = profile.username;
      user.discordAvatar = profile.avatar;
      await db.set('users', users);
...
router.get("/auth/logout", (req, res, next) => {
  console.log("NUCLEAR LOGOUT for user:", req.user?.username);
  
  // Force security headers immediately
  ...
  if (req.session) {
    ...
    if (req.session.authOrigin) delete req.session.authOrigin;
    req.session.authenticated = false;
    req.session.isLoggedOut = true; // Add marker to detect logout state
  }
  
  // Clear all cookies with multiple variations to catch every possible cookie
  const cookieNames = ['app.session', 'connect.sid', 'sid', 'express:sess', 'express.sid'];
  const cookiePaths = ['/', '/instances', '/instance', '/auth', ''];
  const cookieDomains = [null, req.hostname, 'localhost', '127.0.0.1'];
  ...
  req.logout((err) => {
    ...
    if (req.session) {
      req.session.destroy((err) => {
        ...
        console.log("🧨 NUCLEAR LOGOUT COMPLETE - ALL SESSION DATA DESTROYED");
        
        // Redirect with timestamp to prevent caching
        const timestamp = Date.now();
        const redirectUrl = `/login?msg=LogoutSuccess&nuclear=true&t=${timestamp}`;
        
        res.redirect(redirectUrl);
      });
    } else {
      // If no session exists, redirect anyway
      const timestamp = Date.now();
      res.redirect(`/login?msg=LogoutSuccess&noSession=true&t=${timestamp}`);
    }
  });
});
